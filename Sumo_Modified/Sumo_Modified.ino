...
void lineSensors(int speed, int ms) { // Sequence for line avoidance.
  if(digitalRead(lineSensor_1) == HIGH && digitalRead(lineSensor_2) == HIGH) { // Front Two.
    ...
    bwLt(100);
    delay(ms/2);
  } else if(digitalRead(lineSensor_3) == LOW && digitalRead(lineSensor_1) == HIGH) { // Front Left and Back.
    onLine = true;
    tnRt(speed);
    delay(ms);
  } else if(digitalRead(lineSensor_3) == LOW && digitalRead(lineSensor_2) == HIGH) { // Front Right and Back.
    onLine = true;
    tnLt(speed);
    delay(ms);
  } else if(digitalRead(lineSensor_3) == LOW) { // Just Back.
    onLine = true;
    mvFwd(100);
    delay(ms);
